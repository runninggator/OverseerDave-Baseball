...
if (!isset($_POST['opponent'])) header("Location: online.php");
...
$opponent = getUser($_POST['opponent']);
$opponentTeam = $opponent->getTeamname();

$playerA = (float)$user->analyzeUser();
$playerB = (float)$opponent->analyzeUser();
//echo "Player A: $playerA<br>Player B: $playerB<br>";
...
if ($playerAScore[9] > $playerBScore[9])
{
	...
	$opponent->setMoney($opponent->getMoney() + 1);
...
else
{
	...
	$user->setExperience($user->getExperience() + $exp);
	$user->setMoney($user->getMoney() + $mon);
	$opponent->setMoney($opponent->getMoney() + 2);
					...
					<div class='col-sm-10 col-sm-offset-1 col-xs-12'>
						<div class="panel panel-default">
							<div class='panel-heading'>
								<h3 class='panel-title text-center'>Box Score - <b>$teamname vs $opponentTeam</b></h3>
							</div>
...
echo "<tr>";
echo "<td>" . $opponent->getTeamname() . "</td>";
for ($i = 0; $i < 12; $i++)
{
	if ($i < 9)
	{
